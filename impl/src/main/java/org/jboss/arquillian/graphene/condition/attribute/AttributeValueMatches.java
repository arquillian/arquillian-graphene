...
public class AttributeValueMatches extends AbstractAttributeAndValueBooleanCondition {
    ...
    public AttributeValueMatches(WebElement element, String attribute, String value) {
        super(element, attribute, value);
    }

    public AttributeValueMatches(WebElement element, String attribute, String value, boolean negation) {
    ...
    protected Boolean check(WebDriver driver) {
        ...
        return found != null && found.matches(getValue());
    }

    @Override
    public String toString() {
        return String.format("text ('%s') to%s match value ('%s') of attribute ('%s') in element %s",
            getValue(),
            (getNegation() ? " not" : ""),

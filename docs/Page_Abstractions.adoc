...
* link:#53117992[Page Objects]
* link:#53117994[Page Fragments]
* link:#82903669[Location of pages]

[[53117989_PageAbstractions-PageAbstractionsenhancethewayyourtestsarewritten.Theymakeyourtestsmorerobustandthereforemorereliable.]]
...
To see their power let's start with little bit of motivation. Do you
...
page will change (it can be even slight change in class attribute of
some element) tests are affected as well. These small changes can
effectively break lot of tests, in other words increases the time spent
on the tests maintenance.

[[53117989_PageAbstractions-TherecomesPageObjectspatterntoimprovethis.]]
...
The test can be now decoupled from the underlying HTML structure of the
tested page. Once the structure change, the only modification will be
...
please follow the child page link:#53117992[Page Objects].

Page Objects pattern are well known concept, which greatly improves
tests robustness. However, is there more space for improvement ?
Consider previous example, where we were testing three autocomplete
...
autocomplete widget implemented with the same UI framework. It is a huge
DRY violation.

[[53117989_PageAbstractions-ThereforeGrapheneintroducesanewconcept%2CcalledPageFragmentstoimprovethis.]]
...
* A concept which differentiate each fragment by its root element and
make other parts referenced from it.
...
denote whether it is Page Fragment or Page Object in the way you use it
...
structure of the testing page, less preffered).
...
and more, please continue with link:#53117994[Page Fragments].
...
automatically enrich the Page Object's injection points (e.g.
`@FindBy`).
* Page Objects can contain link:#53117994[Page Fragments] and they are
...
* Page Objects can
https://docs.jboss.org/author/display/ARQGRA2/Location+of+pages[encapsulate
their location]

You have already saw the default way of initializing Page Objects in the
parent link:#53117989[Page Abstractions] page. *Let's showcase Graphene
way of doing this!*
...
To see it in action, consider please code snippets below, which
demonstrates testing of a simple web page with progress bar and two
buttons.

[[53117992_PageObjects-ImplementPageObject]]
...
in the same way inject your link:#53117994[Page Fragments], or even
`java.util.List` of Page Fragments/WebElements.

Note that you have to use private fields for all Graphene initialized
...
sure to
check 
https://docs.jboss.org/author/display/ARQGRA2/Parallel+browsers#Parallelbrowsers-UsingPageAbstractionswithMultipleBrowsers[Using
Page Abstractions with Multiple Browsers].

[[53117992_PageObjects-Declareitinthetestandannotateitwith%7B%7B%40Page%7D%7Dannotation.]]
...
[[53117992_PageObjects-Deploymentandbrowserdetermination]]
...
Graphene support for
https://docs.jboss.org/author/display/ARQGRA2/Parallel+browsers[parallel
browsers] for more information about this subject.
...
introduced in link:#53117989[Page Abstractions]. Here you can find more
detailed info about:

1.  link:#53117994_PageFragments-RelationtoPageObjects[Relation to Page
Objects]
2.  link:#53117994_PageFragments-Usage[Usage]
3.  link:#53117994_PageFragments-CreatingPageFragments[Creating Page
Fragments]
4.  link:#53117994_PageFragments-SamplesofPageFragments[Samples of Page
Fragments]
1.  link:#53117994_PageFragments-Autocomplete[Autocomplete]
2.  link:#53117994_PageFragments-Calendar[Calendar]
3.  link:#53117994_PageFragments-Table[Table]
4.  link:#53117994_PageFragments-Logincomponent[Login component]

[[53117994_PageFragments-RelationtoPageObjects]]
[[relation-to-page-objects]]
Relation to Page Objects
~~~~~~~~~~~~~~~~~~~~~~~~

Page Fragments and Page Objects are similar concepts. They both
decouples HTML structure of the tested application from the tests. They
...
[[53117994_PageFragments-CreatingPageFragments]]
...
+
If you need to access the injected root of your Page Fragment, you ca do
...
with the xpath locators. They can be written in a way that it does not
matter from which element they are referenced. Prefer therefore locating
according to id, css or class name.

Handy Hint

Indeed you will need a reference for `WebDriver` instance. You can
...
[[53117994_PageFragments-SamplesofPageFragments]]
...
[[53117994_PageFragments-Autocomplete]]
[[autocomplete]]
Autocomplete
^^^^^^^^^^^^
...
[[53117994_PageFragments-Calendar]]
[[calendar]]
Calendar
^^^^^^^^
...
[[53117994_PageFragments-Logincomponent]]
[[login-component]]
Login component
^^^^^^^^^^^^^^^
...
[[82903669_Locationofpages-Locationconcepts]]
[[location-concepts]]
Location concepts
~~~~~~~~~~~~~~~~~

Page object encapsulates some page. This page has its location. We can
see location of a page as some kind of metadata logically related to
...
[[82903669_Locationofpages-Locationschemes]]
[[location-schemes]]
Location schemes
~~~~~~~~~~~~~~~~

Every location has some scheme. In the previous example, there is used
HTTP scheme by default. There are multiple schemes as:

[[82903669_Locationofpages-HTTPscheme]]
[[http-scheme]]
HTTP scheme
^^^^^^^^^^^

Treats location as http:// scheme. You do not have to specify _scheme_
paramater since by default it is HTTP
...
[[82903669_Locationofpages-Filescheme]]
[[file-scheme]]
File scheme
^^^^^^^^^^^
...
[[82903669_Locationofpages-Resourcescheme]]
[[resource-scheme]]
Resource scheme
^^^^^^^^^^^^^^^
...
[[82903669_Locationofpages-HowdoIimplementmyownscheme%3F]]
...
which will WebDriver instance subsequently open via get() method.
...
[[82903669_Locationofpages-Howtosetdefaultschemeperproject%3F]]
[[how-to-set-default-scheme-per-project]]
How to set default scheme per project?
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Imaging you are dealing only with Android activities so specifying

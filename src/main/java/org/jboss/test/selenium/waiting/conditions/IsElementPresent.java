...
public class IsElementPresent implements Condition, JavaScriptCondition, Contextual {
	...
	public IsElementPresent locator(ElementLocator elementLocator) {
		Validate.notNull(elementLocator);

		IsElementPresent copy = copy();
		copy.elementLocator = elementLocator;

		return copy;
	}

	private IsElementPresent copy() {
		IsElementPresent copy = new IsElementPresent();
		copy.elementLocator = elementLocator;
		return copy;
	}
}
